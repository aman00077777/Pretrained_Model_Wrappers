"""Unit tests for the ConvNeXT Vision encoder wrapper."""
...
@pytest.fixture
def fake_model() -> FakeConvNextModel:
    return FakeConvNextModel(hidden_size=HIDDEN_SIZE)


@pytest.fixture
def fake_processor() -> FakeConvNextProcessor:
    return FakeConvNextProcessor()


@pytest.fixture
def encoder(fake_model: FakeConvNextModel, fake_processor: FakeConvNextProcessor) -> ConvNextEncoder:
    with patch(
        "fusion.models.pretrained.vision.convnext.AutoImageProcessor.from_pretrained",
        return_value=fake_processor,
    ), patch(
        "fusion.models.pretrained.vision.convnext.ConvNextModel.from_pretrained",
        return_value=fake_model,
    ):
        return ConvNextEncoder.from_pretrained("facebook/convnext-base-224-22k")


def test_wrapper_loads_model_successfully(fake_model: FakeConvNextModel, fake_processor: FakeConvNextProcessor) -> None:
    ...
    with patch(
        ...
        enc = ConvNextEncoder("facebook/convnext-base-224-22k")

    mock_proc.assert_called_once()
    mock_model.assert_called_once()
    assert isinstance(enc, ConvNextEncoder)
    assert enc.get_output_dim() == HIDDEN_SIZE


def test_encode_returns_modal_tensor_type(encoder: ConvNextEncoder) -> None:
...
def test_encode_output_shape_matches_expected_dim(encoder: ConvNextEncoder) -> None:
...
def test_encode_batch_of_images_returns_batch_dim(encoder: ConvNextEncoder) -> None:
...
def test_encode_single_image_handled_correctly(encoder: ConvNextEncoder) -> None:
...
def test_invalid_image_input_raises_clear_error(encoder: ConvNextEncoder) -> None:
