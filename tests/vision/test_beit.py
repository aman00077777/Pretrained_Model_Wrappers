"""Unit tests for the BEiT Vision encoder wrapper."""
...
class FakeConfig:
    """Fake config for Hugging Face BEiT."""

    def __init__(self, hidden_size: int = HIDDEN_SIZE) -> None:
        self.hidden_size = hidden_size


class FakeBeitModel(nn.Module):
    """Fake BEiT Model."""
...
@pytest.fixture
def fake_model() -> FakeBeitModel:
    return FakeBeitModel(hidden_size=HIDDEN_SIZE)


@pytest.fixture
def fake_processor() -> FakeBeitProcessor:
    return FakeBeitProcessor()


@pytest.fixture
def encoder(fake_model: FakeBeitModel, fake_processor: FakeBeitProcessor) -> BeitEncoder:
    with patch(
        "fusion.models.pretrained.vision.beit.AutoImageProcessor.from_pretrained",
        return_value=fake_processor,
    ), patch(
        "fusion.models.pretrained.vision.beit.BeitModel.from_pretrained",
        return_value=fake_model,
    ):
        return BeitEncoder.from_pretrained("microsoft/beit-base-patch16-224")


def test_wrapper_loads_model_successfully(fake_model: FakeBeitModel, fake_processor: FakeBeitProcessor) -> None:
    ...
    with patch(
        ...
        enc = BeitEncoder("microsoft/beit-base-patch16-224")

    mock_proc.assert_called_once()
    mock_model.assert_called_once()
    assert isinstance(enc, BeitEncoder)
    assert enc.get_output_dim() == HIDDEN_SIZE


def test_encode_returns_modal_tensor_type(encoder: BeitEncoder) -> None:
...
def test_encode_output_shape_matches_expected_dim(encoder: BeitEncoder) -> None:
...
def test_encode_batch_of_images_returns_batch_dim(encoder: BeitEncoder) -> None:
...
def test_encode_single_image_handled_correctly(encoder: BeitEncoder) -> None:
...
def test_invalid_image_input_raises_clear_error(encoder: BeitEncoder) -> None:
