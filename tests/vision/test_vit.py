"""Unit tests for the ViT Vision encoder wrapper."""
...
class FakeConfig:
    """Fake config for Hugging Face ViT."""
...
@pytest.fixture
def fake_model() -> FakeViTModel:
    return FakeViTModel(hidden_size=HIDDEN_SIZE)


@pytest.fixture
def fake_processor() -> FakeViTProcessor:
    return FakeViTProcessor()


@pytest.fixture
def encoder(fake_model: FakeViTModel, fake_processor: FakeViTProcessor) -> ViTEncoder:
    with patch(
        "fusion.models.pretrained.vision.vit.AutoImageProcessor.from_pretrained",
        return_value=fake_processor,
    ), patch(
        "fusion.models.pretrained.vision.vit.ViTModel.from_pretrained",
        return_value=fake_model,
    ):
        return ViTEncoder.from_pretrained("google/vit-base-patch16-224")


def test_wrapper_loads_model_successfully(fake_model: FakeViTModel, fake_processor: FakeViTProcessor) -> None:
    ...
    with patch(
        ...
        enc = ViTEncoder("google/vit-base-patch16-224")
...
def test_encode_returns_modal_tensor_type(encoder: ViTEncoder) -> None:
...
def test_encode_output_shape_matches_expected_dim(encoder: ViTEncoder) -> None:
...
def test_encode_batch_of_images_returns_batch_dim(encoder: ViTEncoder) -> None:
...
def test_encode_single_image_handled_correctly(encoder: ViTEncoder) -> None:
...
def test_invalid_image_input_raises_clear_error(encoder: ViTEncoder) -> None:
