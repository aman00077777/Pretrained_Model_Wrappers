"""Unit tests for the EfficientNet Vision encoder wrapper."""
...
class FakeConfig:
    """Fake config for Hugging Face EfficientNet."""

    def __init__(self, hidden_dim: int = HIDDEN_SIZE) -> None:
        self.hidden_dim = hidden_dim


class FakeEfficientNetModel(nn.Module):
    """Fake EfficientNet Model."""

    def __init__(self, hidden_size: int = HIDDEN_SIZE) -> None:
        super().__init__()
        self.config = FakeConfig(hidden_dim=hidden_size)
        self.param = nn.Parameter(torch.zeros(1))

    def forward(self, pixel_values: torch.Tensor, **kwargs: Any) -> SimpleNamespace:
        B = pixel_values.shape[0]
        C = self.config.hidden_dim
        # EfficientNet pooler output is 4D (B, C, 1, 1)
        return SimpleNamespace(pooler_output=torch.randn(B, C, 1, 1))


class FakeEfficientNetProcessor:
    """Fake EfficientNet Processor."""
...
@pytest.fixture
def fake_model() -> FakeEfficientNetModel:
    return FakeEfficientNetModel(hidden_size=HIDDEN_SIZE)


@pytest.fixture
def fake_processor() -> FakeEfficientNetProcessor:
    return FakeEfficientNetProcessor()


@pytest.fixture
def encoder(fake_model: FakeEfficientNetModel, fake_processor: FakeEfficientNetProcessor) -> EfficientNetEncoder:
    with patch(
        "fusion.models.pretrained.vision.efficientnet.AutoImageProcessor.from_pretrained",
        return_value=fake_processor,
    ), patch(
        "fusion.models.pretrained.vision.efficientnet.EfficientNetModel.from_pretrained",
        return_value=fake_model,
    ):
        return EfficientNetEncoder.from_pretrained("google/efficientnet-b0")


def test_wrapper_loads_model_successfully(fake_model: FakeEfficientNetModel, fake_processor: FakeEfficientNetProcessor) -> None:
    ...
    with patch(
        ...
        enc = EfficientNetEncoder("google/efficientnet-b0")

    mock_proc.assert_called_once()
    mock_model.assert_called_once()
    assert isinstance(enc, EfficientNetEncoder)
    assert enc.get_output_dim() == HIDDEN_SIZE


def test_encode_returns_modal_tensor_type(encoder: EfficientNetEncoder) -> None:
...
def test_encode_output_shape_matches_expected_dim(encoder: EfficientNetEncoder) -> None:
...
def test_encode_batch_of_images_returns_batch_dim(encoder: EfficientNetEncoder) -> None:
...
def test_encode_single_image_handled_correctly(encoder: EfficientNetEncoder) -> None:
...
def test_invalid_image_input_raises_clear_error(encoder: EfficientNetEncoder) -> None:
