"""Unit tests for the ResNet Vision encoder wrapper."""
...
class FakeConfig:
    """Fake config for Hugging Face ResNet."""

    def __init__(self, hidden_sizes: list[int]) -> None:
        self.hidden_sizes = hidden_sizes


class FakeResNetModel(nn.Module):
    """Fake ResNet Model."""
    ...
    def forward(self, pixel_values: torch.Tensor, **kwargs: Any) -> SimpleNamespace:
        ...
        # ResNet pooler output is 4D (B, C, 1, 1)
        return SimpleNamespace(pooler_output=torch.randn(B, C, 1, 1))


class FakeResNetProcessor:
    """Fake ResNet Processor."""
...
@pytest.fixture
def fake_model() -> FakeResNetModel:
    return FakeResNetModel(hidden_size=HIDDEN_SIZE)


@pytest.fixture
def fake_processor() -> FakeResNetProcessor:
    return FakeResNetProcessor()


@pytest.fixture
def encoder(fake_model: FakeResNetModel, fake_processor: FakeResNetProcessor) -> ResNetEncoder:
    with patch(
        "fusion.models.pretrained.vision.resnet.AutoImageProcessor.from_pretrained",
        return_value=fake_processor,
    ), patch(
        "fusion.models.pretrained.vision.resnet.ResNetModel.from_pretrained",
        return_value=fake_model,
    ):
        return ResNetEncoder.from_pretrained("microsoft/resnet-50")


def test_wrapper_loads_model_successfully(fake_model: FakeResNetModel, fake_processor: FakeResNetProcessor) -> None:
    ...
    with patch(
        ...
        enc = ResNetEncoder("microsoft/resnet-50")
...
def test_encode_returns_modal_tensor_type(encoder: ResNetEncoder) -> None:
...
def test_encode_output_shape_matches_expected_dim(encoder: ResNetEncoder) -> None:
...
def test_encode_batch_of_images_returns_batch_dim(encoder: ResNetEncoder) -> None:
...
def test_encode_single_image_handled_correctly(encoder: ResNetEncoder) -> None:
...
def test_invalid_image_input_raises_clear_error(encoder: ResNetEncoder) -> None:
