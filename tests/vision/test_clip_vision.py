"""Unit tests for the CLIP Vision encoder wrapper."""
...
@pytest.fixture
def fake_model() -> FakeCLIPVisionModel:
    return FakeCLIPVisionModel(hidden_size=HIDDEN_SIZE)


@pytest.fixture
def fake_processor() -> FakeCLIPProcessor:
    return FakeCLIPProcessor()


@pytest.fixture
def encoder(fake_model: FakeCLIPVisionModel, fake_processor: FakeCLIPProcessor) -> CLIPVisionEncoder:
    with patch(
        "fusion.models.pretrained.vision.clip_vision.AutoProcessor.from_pretrained",
        return_value=fake_processor,
    ), patch(
        "fusion.models.pretrained.vision.clip_vision.CLIPVisionModel.from_pretrained",
        return_value=fake_model,
    ):
        return CLIPVisionEncoder.from_pretrained("openai/clip-vit-base-patch32")


def test_wrapper_loads_model_successfully(fake_model: FakeCLIPVisionModel, fake_processor: FakeCLIPProcessor) -> None:
    ...
    with patch(
        ...
        enc = CLIPVisionEncoder("openai/clip-vit-base-patch32")
...
def test_encode_returns_modal_tensor_type(encoder: CLIPVisionEncoder) -> None:
...
def test_encode_output_shape_matches_expected_dim(encoder: CLIPVisionEncoder) -> None:
...
def test_encode_batch_of_images_returns_batch_dim(encoder: CLIPVisionEncoder) -> None:
...
def test_encode_single_image_handled_correctly(encoder: CLIPVisionEncoder) -> None:
...
def test_invalid_image_input_raises_clear_error(encoder: CLIPVisionEncoder) -> None:
