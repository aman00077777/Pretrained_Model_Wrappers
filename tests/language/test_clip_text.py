...
class TestCLIPTextEncoder:
    def _load(self, fake_tokenizer, make_fake_model, **kwargs) -> CLIPTextEncoder:
        with patch(
            "fusion.encoders.language.clip_text.CLIPTokenizer.from_pretrained",
            return_value=fake_tokenizer,
        ), patch(
            "fusion.encoders.language.clip_text.CLIPTextModelWithProjection.from_pretrained",
            return_value=make_fake_model(
                hidden_size=PROJECTION_DIM, has_pooler=False, has_text_embeds=True,
                projection_dim=PROJECTION_DIM,
            ),
        ):
            return CLIPTextEncoder.from_pretrained("openai/clip-vit-base-patch32", **kwargs)
    ...
    def test_wrapper_loads_tokenizer_and_model(self, fake_tokenizer, make_fake_model) -> None:
        ...
        with patch(
            ...
            "fusion.encoders.language.clip_text.CLIPTextModelWithProjection.from_pretrained",
            return_value=make_fake_model(
                hidden_size=PROJECTION_DIM, has_text_embeds=True, projection_dim=PROJECTION_DIM,
            ),
        ) as mock_model:
            encoder = CLIPTextEncoder.from_pretrained("openai/clip-vit-base-patch32")
    ...
    def test_encode_long_text_truncates_without_error(self, fake_tokenizer, make_fake_model) -> None:
        """Text far longer than CLIP's 77-token context must truncate, not error."""
        encoder = self._load(fake_tokenizer, make_fake_model)
        long_text = "word " * 500
        result = encoder.encode([long_text])  # default max_length=77
        assert result.tensor.shape[0] == 1
        assert torch.isfinite(result.tensor).all()
