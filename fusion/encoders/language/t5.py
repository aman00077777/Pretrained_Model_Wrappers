"""Task S4 — T5 encoder. Uses T5EncoderModel only (no decoder)."""
import logging
import time
...
class T5Encoder(BaseEncoder):
    def __init__(self, config: dict = None):
        ...
        log_event(logger, logging.INFO, "model_loading", encoder="t5", model=model_name)
        self.model = T5EncoderModel.from_pretrained(model_name)
        self.tokenizer = AutoTokenizer.from_pretrained(model_name)
        self._output_dim = self.model.config.d_model
        log_event(logger, logging.INFO, "initialization_complete", encoder="t5",
    ...
    def encode(self, inputs) -> ModalTensor:
        if isinstance(inputs, str):
            inputs = [inputs]
        if not inputs:
            log_event(logger, logging.ERROR, "encoding_failed", encoder="t5",
                       error_type="ValueError")
            raise ValueError("encode() received an empty batch")

        start = time.time()
        log_event(logger, logging.DEBUG, "encode_started", encoder="t5", batch_size=len(inputs))

        try:
            tokenized = self.tokenizer(
                inputs, padding=True, truncation=True, max_length=512, return_tensors="pt"
            )
            log_event(logger, logging.DEBUG, "preprocessing_complete", encoder="t5")

            outputs = self.model(
                input_ids=tokenized["input_ids"], attention_mask=tokenized["attention_mask"]
            )
            log_event(logger, logging.DEBUG, "forward_complete", encoder="t5")
        ...
        log_event(logger, logging.DEBUG, "encode_complete", encoder="t5",
