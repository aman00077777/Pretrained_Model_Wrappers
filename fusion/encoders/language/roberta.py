"""Task S3 — RoBERTa encoder. Same masked mean-pooling as BERT; no token_type_ids."""
import logging
import time
...
@ENCODER_REGISTRY.register("roberta")
class RoBERTaEncoder(BaseEncoder):
    def __init__(self, config: dict = None):
        ...
        log_event(logger, logging.INFO, "model_loading", encoder="roberta", model=model_name)
        self.model = AutoModel.from_pretrained(model_name)
        self.tokenizer = AutoTokenizer.from_pretrained(model_name)
        self._output_dim = self.model.config.hidden_size
        log_event(logger, logging.INFO, "initialization_complete", encoder="roberta",
    ...
    def encode(self, inputs) -> ModalTensor:
        if isinstance(inputs, str):
            inputs = [inputs]
        if not inputs:
            log_event(logger, logging.ERROR, "encoding_failed", encoder="roberta",
                       error_type="ValueError")
            raise ValueError("encode() received an empty batch")

        start = time.time()
        log_event(logger, logging.DEBUG, "encode_started", encoder="roberta", batch_size=len(inputs))

        try:
            tokenized = self.tokenizer(
                inputs, padding=True, truncation=True, max_length=512, return_tensors="pt"
            )
            tokenized.pop("token_type_ids", None)  # RoBERTa does not use token type IDs
            log_event(logger, logging.DEBUG, "preprocessing_complete", encoder="roberta")

            outputs = self.model(
                input_ids=tokenized["input_ids"], attention_mask=tokenized["attention_mask"]
            )
            log_event(logger, logging.DEBUG, "forward_complete", encoder="roberta")
        ...
        log_event(logger, logging.DEBUG, "encode_complete", encoder="roberta",
