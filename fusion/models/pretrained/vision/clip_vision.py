"""CLIP Vision language encoder wrapper.
...
class CLIPVisionEncoder(BaseEncoder):
    """Wraps Hugging Face's CLIP vision model as a FUSION vision encoder."""
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        logger.info("event=model_loading encoder=clip_vision model=%s", model_name)
        try:
            processor = AutoProcessor.from_pretrained(model_name, cache_dir=cache_dir)
            model = CLIPVisionModel.from_pretrained(model_name, cache_dir=cache_dir, **kwargs)
        except Exception as e:
            logger.error("event=model_loading_failed encoder=clip_vision model=%s error=%s", model_name, str(e))
            raise EncoderError(f"Failed to load CLIP model '{model_name}': {e}") from e
        ...
        logger.info("event=initialization_complete encoder=clip_vision output_dim=%d", self._output_dim)
    ...
    def encode(
        ...
        if isinstance(images, torch.Tensor):
            ...
            try:
                ...
                logger.error("event=preprocessing_failed encoder=clip_vision error=%s", str(e))
                raise DataError(f"Failed to preprocess images: {e}") from e

        device = next(self.model.parameters()).device
        pixel_values = pixel_values.to(device)

        logger.debug("event=encode_started encoder=clip_vision batch_size=%d", batch_size)
        logger.debug("event=preprocessing_complete encoder=clip_vision")

        try:
            with torch.no_grad():
                outputs = self.model(pixel_values=pixel_values)
            logger.debug("event=forward_complete encoder=clip_vision")
        except Exception as e:
            logger.error("event=encoding_failed encoder=clip_vision error_type=%s", type(e).__name__)
        ...
        logger.debug(
            "event=encode_complete encoder=clip_vision output_shape=%s duration_ms=%.2f",
            list(pooled.shape),
            duration_ms,
        )
