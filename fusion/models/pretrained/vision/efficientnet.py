"""EfficientNet vision encoder wrapper.
...
class EfficientNetEncoder(BaseEncoder):
    """Wraps Hugging Face's EfficientNet model as a FUSION vision encoder."""
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        logger.info("event=model_loading encoder=efficientnet model=%s", model_name)
        try:
            processor = AutoImageProcessor.from_pretrained(model_name, cache_dir=cache_dir)
            model = EfficientNetModel.from_pretrained(model_name, cache_dir=cache_dir, **kwargs)
        except Exception as e:
            logger.error("event=model_loading_failed encoder=efficientnet model=%s error=%s", model_name, str(e))
            raise EncoderError(f"Failed to load EfficientNet model '{model_name}': {e}") from e

        self._output_dim = model.config.hidden_dim
        ...
        logger.info("event=initialization_complete encoder=efficientnet output_dim=%d", self._output_dim)
    ...
    def encode(
        ...
        if isinstance(images, torch.Tensor):
            ...
            try:
                ...
                logger.error("event=preprocessing_failed encoder=efficientnet error=%s", str(e))
                raise DataError(f"Failed to preprocess images: {e}") from e

        device = next(self.model.parameters()).device
        pixel_values = pixel_values.to(device)

        logger.debug("event=encode_started encoder=efficientnet batch_size=%d", batch_size)
        logger.debug("event=preprocessing_complete encoder=efficientnet")

        try:
            with torch.no_grad():
                outputs = self.model(pixel_values=pixel_values)
            logger.debug("event=forward_complete encoder=efficientnet")
        except Exception as e:
            logger.error("event=encoding_failed encoder=efficientnet error_type=%s", type(e).__name__)
        ...
        logger.debug(
            "event=encode_complete encoder=efficientnet output_shape=%s duration_ms=%.2f",
            list(pooled.shape),
            duration_ms,
        )
